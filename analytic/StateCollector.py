...
def collect_samples(agent, path, dest, mode):
    ...
    states = data['states'][:1000]
    next_states = data['next_states'][:1000]
    ...
    # sns.heatmap(dist.numpy(), cmap='coolwarm', vmin=0, vmax=60)
    # plt.savefig('./states.png')
    #
    # dist = torch.cdist(torch.tensor(feature), torch.tensor(feature), p=2)
    # plt.figure(figsize=(20.48, 20.48))
    # sns.heatmap(dist.numpy(), cmap='coolwarm', vmin=0, vmax=60)
    # plt.savefig('./{0:s}_features.png'.format(dest.name))
